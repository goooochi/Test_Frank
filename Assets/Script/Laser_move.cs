...
public class Laser_move : MonoBehaviour {
    ...
    {
        if (collision.gameObject.tag == "Cube")//壁と当たった時
        {
            Vector3 refrectVec = Vector3.Reflect(this.lastVelocity, collision.contacts[0].normal);//反射ベクトル計算
            this.rb.velocity = refrectVec;
        }
        if (collision.gameObject.tag == "Cube_Destroy")//破壊出来る壁に当たった時
        {
            Destroy(collision.gameObject);//壁削除
            Vector3 refrectVec = Vector3.Reflect(this.lastVelocity, collision.contacts[0].normal);//反射ベクトル計算
            this.rb.velocity = refrectVec;
        }
        //if (collision.gameObject.tag == "Player_Cube")//プレイヤーと当たった時
        //{
        //    Vector3 refrectVec = Vector3.Reflect(this.lastVelocity, collision.contacts[0].normal);//反射ベクトル計算
        //    this.rb.velocity = refrectVec;
